...
async function createTenantTables(connection) {
  ...
  // Asset Categories table (managed list of CMDB asset categories)
  await connection.execute(`
  ...
  await connection.execute(`
    CREATE TABLE IF NOT EXISTS email_ingest_settings (
      ...
      m365_enabled TINYINT(1) DEFAULT 0,
      m365_use_for_outbound TINYINT(1) DEFAULT 0,
  ...
  await connection.execute(`
    CREATE TABLE IF NOT EXISTS ticket_processing_rules (
      ...
      source_filter ENUM('all', 'monitoring', 'customer') DEFAULT 'all',
